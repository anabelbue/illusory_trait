...
create_label_map <- function(state_comp, other_constructs_map, trait_map) {
  ...
  abbrev <- c(
    ...
    tiredness = "tir", typicality = "typ", stress = "str", negativity = "neg",
    hunger = "hun", duty = "dut", sociability = "soc",  happiness = "hap", 
...
write_xlsx(tables_labeled, "full_output_tables_S2.xlsx")
...
fit_table_S2   <- create_fit_table(results_all, fit_measures_all)
conv_table_S2  <- create_conv_table(results_all)
nearPD_table_S2 <- create_nearPD_table(cov_checks_detrend, state_components)

write_xlsx(
  list(
    "Fit Indices" = fit_table_S2,
    "Convergence" = conv_table_S2,
    "nearPD" = nearPD_table_S2
  ),
  "diagnostic_tables_S2.xlsx"
)
