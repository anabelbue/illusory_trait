...
# Time points run from 3 to n_time (=34) for Sample 1.
generate_mv_riclpm_syntax <- function(construct_names, n_time, trait_names = NULL) {
  ...
  for (k in 1:n_c) {
    loadings <- paste0("1*", construct_names[k], "_", 3:n_time, collapse = " + ")
  ...
  for (k in 1:n_c) {
    ...
    for (t in 4:n_time) {
      lag_label <- ifelse((t - 1) %% 8 == 0, "0", ar_label)
  ...
  if (n_c > 1) {
    ...
    for (k in 1:n_c) {
      for (j in 1:n_c) {
        ...
        for (t in 4:n_time) {
          lag_label <- ifelse((t - 1) %% 8 == 0, "0", cl_label)
          lines <- c(lines, paste0("w", clabs[k], "_", t, " ~ ", lag_label,
                                   "*w", clabs[j], "_", t-1))
        }
      }
  ...
  # ---- Variance wave 3 (free per construct) ----
  lines <- c(lines, "# === Variance Wave 3 (free) ===")
  for (k in 1:n_c) {
    lines <- c(lines, paste0("w", clabs[k], "_3 ~~ v1_", clabs[k],
                             "*w", clabs[k], "_3"))
  }
  lines <- c(lines, "")
  
  # ---- Covariances wave 3 (free) ----
  if (n_c > 1) {
    lines <- c(lines, "# === Covariances Wave 3 (free) ===")
    for (k in 1:(n_c - 1)) {
      for (j in (k + 1):n_c) {
        lines <- c(lines, paste0("w", clabs[k], "_3 ~~ cov1_", clabs[k], clabs[j],
                                 "*w", clabs[j], "_3"))
      }
    }
    lines <- c(lines, "")
...
for (state_comp in state_components) {
  ...
  for (n_add in 0:length(other_csts)) {
    ...
    for (cst in current_constructs) {
      all_vars <- c(all_vars, paste0(cst, "_", 3:n_time))
    }
