...
vars_with_na <- names(d1)[colSums(is.na(d1)) > 0]
...
for (v in vars_with_na) {
  tryCatch({
    ...
    dens_true <- density(d1[[v]], na.rm = TRUE)
...
other_constructs_map <- list(
  ...
  state_n = c("happiness", "sociability", "positivity", "negativity", "mating"),
  state_o = c("happiness", "sociability", "positivity", "negativity", "mating"),
  state_a = c("happiness", "sociability", "positivity", "negativity", "mating"),
  state_c = c("happiness", "sociability", "positivity", "negativity", "mating")
)
